...
func getBeers() ([]Beer, error) {
	...
	defer db.Close()
	rows, err := db.Query("SELECT id, title, name FROM beer")
	if err != nil {
		return beers, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&beer.ID, &title, &name); err != nil {
			return beers, err
		}

		if name.Valid {
			beer.Title = name.String
		} else {
			beer.Title = title.String
		}
...
func createBeer(toCreate CreateBeer) (Beer, error) {
	beer := Beer{ID: uuid.NewV4(), Title: toCreate.Title}
	...
	if _, err := db.Query("INSERT INTO beer (id, title, name) VALUES($1, $2, $3)", beer.ID, beer.Title, beer.Title); err != nil {
		return beer, err
	}
