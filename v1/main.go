...
func getBeers() ([]Beer, error) {
	...
	defer db.Close()
	rows, err := db.Query("SELECT id, title FROM beer")
	if err != nil {
		return beers, err
	}
	...
	for rows.Next() {
		var beer Beer
		if err := rows.Scan(&beer.ID, &beer.Title); err != nil {
			return beers, err
		}
...
func createBeer(toCreate CreateBeer) (Beer, error) {
	...
	if _, err := db.Query("INSERT INTO beer (id, title) VALUES($1, $2)", beer.ID, beer.Title); err != nil {
		return beer, err
	}
